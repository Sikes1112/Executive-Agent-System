...
Usage:
  apply.py --allowlist contracts/allowlists/canonical_pack_paths.txt --normalized /tmp/iter_normalized_123.json --workspace /path/to/workspace-root
"""
...
def main():
    ...
    for item in bundles:
        ...
        # Defense-in-depth: resolved target must remain inside workspace root
        ws_root = ws.resolve()
        out_resolved = out_path.resolve()
        if ws_root != out_resolved and ws_root not in out_resolved.parents:
            fail("FAIL: resolved path escapes workspace: " + str(out_resolved))

        if rel.endswith(".json"):
            # Fail closed: require full-file object sentinel for JSON writes
            if patch.get("_full_object") is not True:
                fail("FAIL: .json patch missing _full_object:true (prevents partial overwrite): " + rel)
            tmp_path = out_path.with_suffix(out_path.suffix + ".tmp")
            tmp_path.write_text(json.dumps(patch, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
